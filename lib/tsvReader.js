    ...
    if (tsvFilename) {
        glob(`${tsvFilename}.{tsv,txt}`, function (err, files) {
            ...
            if (_.head(files)) {
                ...
                if(column) console.log(parsedTsv.map(object => object[column]));
...
function calculateManagerAnnualSalary(listOfEmployees, manager) {
    ...
    managerCompensations.forEach(worker => {
        if(_.head(worker)[2] === "FT") {
            total += calculateFTAnnualSalary(_.head(worker));
        }

        if(_.head(worker)[2] === "PT") {
            total += calculatePTAnnualSalary(_.head(worker));
        }
    });
    return total;
}